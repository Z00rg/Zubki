...
//TODO: убрать после 14 мая
import { NextResponse } from 'next/server';
import type { NextRequest } from 'next/server';

// 1. Конфигурация путей
const BIRTHDAY_ROUTE = '/birthday';
...
export default function middleware(request: NextRequest) {
    ...
    // Проверяем, не находится ли пользователь уже на нужной странице
    if (pathname === BIRTHDAY_ROUTE) {
    ...
    // Если это не страница поздравления — всегда редирект
    const birthdayUrl = new URL(BIRTHDAY_ROUTE, request.url);
    const response = NextResponse.redirect(birthdayUrl);

    // Добавляем заголовки безопасности к редиректу
    Object.entries(securityHeaders).forEach(([key, value]) => {
        response.headers.set(key, value);
    });
...
export const config = {
    matcher: [
        /*
         * Применяем ко всем путям, кроме системных и статики:
         * - api (запросы к бэкенду)
         * - _next (статика и чанки Next.js)
         * - изображения (чтобы сертификат загрузился!)
         */
        '/((?!api|_next/static|_next/image|favicon.ico|certificate.jpg|.*\\.(?:svg|png|jpg|jpeg|gif|webp)).*)',
    ],
};